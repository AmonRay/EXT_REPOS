...
{
    float2 lv = float2(NdotL, NdotV);
    float2 g1_lv = lv / (lv * (1.0 - a) + a);
    return g1_lv.x * g1_lv.y;
}
...
float G2_Smith_Optimized(float NoL, float NoV, float a2)
{
...
float3 Lit_GGX(float NdotL, float NdotH, float NdotV, float VdotH, float3 F0, float rough)
{
    ...
    float V = G2_Smith(NdotL, NdotV, a2); //denom included?
