...
///////////////////////////////////////////////////////
	...
	// SHADER UNPACKING
		// float lua_param_nvg_generation = floor(heatvision_params3.x);           	  // 1, 2, or 3
		// float lua_param_nvg_num_tubes = frac(heatvision_params3.x) * 10.0f;   	  // 1, 2, 4, 1.1, or 1.2
		// float lua_param_nvg_gain_current = floor(heatvision_params3.y) / 10.0f;	  // 0.0 to 3.0
		// float lua_param_nvg_washout_thresh = frac(heatvision_params3.y);    		  // 0.0 to 0.9
		// float lua_param_vignette_current = floor(heatvision_params3.z) / 100.0f;   // 0.0 to 1.0
		// float lua_param_glitch_power = frac(heatvision_params3.z);        		  // 0.0 to 0.9
		// float lua_param_nvg_gain_offset = floor(heatvision_params3.w) / 10.0f;	  // 0.5 to 3.0
		// float lua_param_nvg_mode = frac(heatvision_params3.w) * 10;                // 0 to 9 in 1.0 increment 
